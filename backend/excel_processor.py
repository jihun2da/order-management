...
def process_excel_file(file_contents: bytes, filename: str, supabase,
    ...
    try:
        ...
        errors   = []
        activity_batch  = []
        status_logs_batch = []   # 상태변경 이력 (롤백 복원용)
            ...
            try:
                ...
                if ex_item:
                    ...
                    if ex_item["status"] != ns:
                        ...
                        # 롤백 복원용 상태 변경 이력 기록
                        status_logs_batch.append({
                            "upload_history_id": upload_id,
                            "order_item_id":     ex_item["id"],
                            "old_status":        ex_item["status"],
                            "new_status":        ns,
                        })
        ...
        # 상태 변경 이력 대량 INSERT (롤백 복원용)
        if status_logs_batch:
            _log(f"[PROC] order_item_status_logs INSERT 중 ({len(status_logs_batch)}건)...")
            for chunk in _chunks(status_logs_batch, BATCH):
                try:
                    supabase.table("order_item_status_logs").insert(chunk).execute()
                except Exception as e:
                    _log(f"[WARN] status_logs insert 실패: {e}")
